    ...
    public void init() {
        ...
        Router router = new Router(new Router.Render() {
            ...
            public boolean render(Context context, Uri uri) {
                Toast.makeText(context, uri.toString() + "*****default*****live", Toast.LENGTH_SHORT).show();
                return true;
            }
        });

        Dns.register("shanks.uri", router);
        router.addRouterDir("/live", new Router.Render() {
            @Override
            public boolean render(Context context, Uri uri) {
                Toast.makeText(context, uri.toString() + "-----default-----live", Toast.LENGTH_SHORT).show();
                return true;
        ...
        router.addRouter("/live/main", new Router.Render() {
            @Override
            public boolean render(Context context, Uri uri) {
